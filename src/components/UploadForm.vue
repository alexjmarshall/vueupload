  ...
  export default {
    ...
    methods: {
      ...
      upload (fileObject) {
        ...
        window.events.$emit('init')

        // this.$http.post('http://localhost:8000/api/upload', form, {
        //   before: (xhr) => {
        //     fileObject.xhr = xhr
        //   },
        //   progress: (e) => {
        //     window.events.$emit('progress', fileObject, e)
        //   }
        // }).then((response) => {
        //   window.events.$emit('finished', fileObject)
        // }, () => {
        //   window.events.$emit('failed', fileObject)
        // })
        ...
        onUploadProgress: function(progressEvent) {
            window.events.$emit('progress', fileObject, progressEvent)
        }
        }).then(function({ data }) {
            window.events.$emit('finished', fileObject)
        })
